          ...
          <>
            <ul className="nav nav-tabs" id="myTab" role="tablist">
              <li className="nav-item navItemCustom" role="presentation">
                <button
                  className="nav-link active navLinkCustom"
                  id="horuly-tab"
                  data-bs-toggle="tab"
                  data-bs-target="#horuly-tab-pane"
                  type="button"
                  role="tab"
                  aria-controls="horuly-tab-pane"
                  aria-selected="true"
                >
                  48-Hour
                </button>
                      ...
                      hourlyInfo.map((fore, i) => (
                        ...
                        >
                          ...
                          {unitsSystem === "imperial" ? (
                            <p className="hourlyDetails hourlyTemp">
                              {Math.round(fore.temp)}°F
                            </p>
                          ) : (
                            <p className="hourlyDetails hourlyTemp">
                              {Math.round(fore.temp)}°C
                            </p>
                          )}
                          <p className="hourlyDetails hourlyPop">
                            {Math.round(fore.precip)}%
                          </p>
                        </div>
                      ))}
                  </div>
                      ...
                      dailyInfo.map((fore, i) => (
                        <div key={i} className="outBoxes">
                          ...
                          <div className={`boxes shadow-sm `}>
                            ...
                            />
                            {unitsSystem === "imperial" ? (
                              <p className="dailyDetails dailyTemp">
                                {Math.round(fore.temp_max)}°F
                              </p>
                            ) : (
                              <p className="dailyDetails dailyTemp">
                                {Math.round(fore.temp_max)}°C
                              </p>
                            )}
                            ...
                            {unitsSystem === "imperial" ? (
                              <p className="dailyDetails dailyTemp">
                                {Math.round(fore.temp_min)}°F
                              </p>
                            ) : (
                              <p className="dailyDetails dailyTemp">
                                {Math.round(fore.temp_min)}°C
                              </p>
                            )}
                            <p className="dailyDetails dailyPop">
                              {Math.round(fore.precip)}%
                            </p>
                          </div>
                        </div>
