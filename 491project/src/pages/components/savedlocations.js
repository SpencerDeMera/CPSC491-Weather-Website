        ...
        {savedLocations &&
          savedLocations.map((location, index) => (
            <div key={index} className="saved-locs">
              <div
                className="saved-locs-inner"
                onClick={() =>
                  handleSelect(index, location.name, location.country)
                }
              >
                {selectedLocation.name === location.name &&
                selectedLocation.country === location.country &&
                currentLocation.lat !== defaultLocation.lat &&
                currentLocation.lon !== defaultLocation.lat ? (
                  <>
                    <a className="locationName" style={{ color: "crimson" }}>
                      {location.name}
                    </a>
                    {location.state && (
                      <a className="locationSub" style={{ color: "crimson" }}>
