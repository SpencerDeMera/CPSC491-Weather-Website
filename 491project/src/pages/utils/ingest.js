...
export const getWeatherAlertData = async (currentLocation) => {
  ...
  try {
    const response = await axios.get(
      `${BASE_URL}/alerts?lat=${currentLocation.lat}&lon=${currentLocation.lon}`
    );
...
export const getTodoData = async (currentLocation, radius_meter, limit) => {
  ...
  if (currentLocation.parkCode !== undefined) {
    ...
    const places_resp = await getPlacesData(
      currentLocation,
      radius_meter,
      limit
    );
  ...
  if (todo.length + places.length < limit) {
    const events_resp = await getEventsData(
      currentLocation,
      limit - todo.length - places.length
    );
...
export const getPlacesData = async (currentLocation, radius_meter, limit) => {
  try {
    const params = {
      ...
      categories:
        "activity,entertainment,leisure,natural,national_park,tourism,camping,amenity",
      conditions: "access",
      lang: "en",
      limit: limit,
    };
...
export const getGooglePhoto = async (place) => {
  ...
  try {
    const response = await axios.get(
      `${BASE_URL}/google/find_photo?input=${textInput}`
    );
